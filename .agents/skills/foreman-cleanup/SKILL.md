---
foreman_version: 0.6.5
name: foreman-cleanup
description: "Play 16 — Cleanup. Detects the project's toolchain, runs auto-fixers (formatter, lint --fix), re-checks types and tests, opens a PR with any cleanup commits, and files deduplicated issues for unfixable failures. Language-agnostic via manifest detection."
...
You are a Foreman skill agent invoked with parameters in `$ARGUMENTS`.
...
1. Read `.foreman/context.json` — repo, owner, learnings.
...
5. Compute the worktree path: `CLEANUP_WORKTREE="$MAIN_REPO/.foreman/worktrees/cleanup-$TARGET"`. Remove any stale worktree: `git worktree remove --force "$CLEANUP_WORKTREE" 2>/dev/null || rm -rf "$CLEANUP_WORKTREE"`.
...
1. If no `tsconfig.json` exists at the worktree root:
   - Skip typecheck entirely. Add `typescript-typecheck` to `tools_skipped` with reason `no tsconfig.json`. Do not file an issue.
2. If `tsconfig.json` exists AND `package.json` contains a `typecheck` script:
   - Run `npm run typecheck`. Record pass/fail.
3. If `tsconfig.json` exists AND no `typecheck` script is present:
   - Probe TypeScript availability: `npx tsc --version >/dev/null 2>&1`.
   - If the probe exits 0: run `npx tsc --noEmit`. Record pass/fail.
   - If the probe exits non-zero (TypeScript not installed): add `tsc-unavailable` to `tools_skipped` with reason `TypeScript not installed in environment`. Do not file a failure issue for the unavailability itself.
