...
test("package.json scripts no longer reference the demoted src/cli implementation", () => {
  ...
  assert.ok(
    !raw.includes("src/cli"),
    "package.json must not reference src/cli",
  );
});
...
test("package-lock.json root bin matches package.json", () => {
  ...
  assert.strictEqual(
    lock.packages[""].bin["timelapse-capture"],
...
test("local-check prints SKIP messages when ffmpeg and ffprobe are absent from PATH", () => {
  ...
  try {
    ...
    assert.strictEqual(
      result.status,
      0,
      `local-check failed; stdout: ${result.stdout}\nstderr: ${result.stderr}`,
    );
    assert.match(
      result.stdout,
...
test("canonical entry uses real Playwright (not the scaffold 1x1 PNG fixture)", () => {
  const cli = readFileSync(
    resolve(REPO_ROOT, "src/timelapse-capture.mjs"),
    "utf8",
  );
...
test("source does not use empty promise catch cleanup handlers", () => {
  ...
  for (const source of sources) {
    ...
    assert.doesNotMatch(
      raw,
      emptyPromiseCatch,
      `${source} must report or propagate cleanup failures`,
    );
  }
});
...
test("source does not use comment-only catch blocks", () => {
  ...
  for (const source of sources) {
    ...
    assert.doesNotMatch(
      raw,
      commentOnlyCatch,
      `${source} must not contain comment-only catch blocks`,
    );
  }
});
...
test("local-check.sh exports only the consumed TIMELAPSE_HAS_REAL_FFMPEG_SUITE env var", () => {
  ...
  assert.ok(
    !script.includes("TIMELAPSE_HAS_REAL_FFMPEG_BINARIES"),
    "TIMELAPSE_HAS_REAL_FFMPEG_BINARIES must not be exported",
  );
  assert.ok(
    !script.includes("TIMELAPSE_HAS_REAL_FFPROBE_BINARIES"),
    "TIMELAPSE_HAS_REAL_FFPROBE_BINARIES must not be exported",
  );
  assert.ok(
    script.includes("TIMELAPSE_HAS_REAL_FFMPEG_SUITE"),
    "TIMELAPSE_HAS_REAL_FFMPEG_SUITE must still be exported",
  );
});
...
test("canonical CLI entrypoint decision is documented and wired", () => {
  const decisionPath = resolve(
    REPO_ROOT,
    "docs/decisions/001-canonical-cli-entrypoint.md",
  );
