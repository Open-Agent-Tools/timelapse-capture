...
async function makeRun(config) {
  ...
  await fs.writeFile(
    ...
    JSON.stringify({ state: "completed", frames: { captured: 1 } }),
  );
  return runDir;
}
...
test("render respects cleanup: never from config.json", async () => {
  ...
  try {
    ...
    const framesExist = await fs
      .access(path.join(runDir, "frames", "frame-0001.png"))
      .then(
        () => true,
        () => false,
      );
...
test("render respects keep-samples from config.json", async () => {
  ...
  await fs.writeFile(
    ...
    JSON.stringify({ keepSamples: 2 }),
  );
  await fs.writeFile(
    path.join(runDir, "status.json"),
    JSON.stringify({ state: "completed", frames: { captured: 3 } }),
  );
  ...
  try {
    ...
    assert.ok(
      samples.every((name) => /^sample-\d+\.png$/.test(name)),
      "Sample filenames should use sample-*.png format",
    );
    const framesExist = await fs.access(framesDir).then(
      () => true,
      () => false,
    );
    assert.equal(
      framesExist,
      false,
      "Frames directory should be removed after keep-samples cleanup",
    );
    ...
    assert.equal(
      summary.cleanup.source,
      "config",
      "Cleanup source should be 'config'",
    );
    assert.equal(
      summary.cleanup.reason,
      "keep-samples",
      "Cleanup reason should be 'keep-samples'",
    );
  } finally {
    await fs.rm(runDir, { recursive: true, force: true });
  }
...
test("render respects keep-latest from config.json", async () => {
  ...
  await fs.writeFile(
    path.join(runDir, "config.json"),
    JSON.stringify({ keepLatest: true }),
  );
  await fs.writeFile(
    path.join(runDir, "status.json"),
    JSON.stringify({ state: "completed", frames: { captured: 2 } }),
  );
  ...
  try {
    ...
    assert.equal(
      summary.cleanup.reason,
      "keep-latest",
      "Cleanup reason should be 'keep-latest'",
    );
  } finally {
    await fs.rm(runDir, { recursive: true, force: true });
  }
