...
test("conflict marker scan includes scripts, docs, and skill text files", async (t) => {
  const tempRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "conflict-marker-scan-"),
  );
  ...
  await fs.writeFile(
    path.join(tempRoot, "scripts", "local-check.sh"),
    "#!/usr/bin/env bash\n",
  );
  await fs.writeFile(path.join(tempRoot, "docs", "PRD.md"), "# PRD\n");
  await fs.writeFile(
    path.join(tempRoot, "docs", "decisions", "001-canonical-cli-entrypoint.md"),
    "# Decision\n",
  );
  await fs.writeFile(
    path.join(tempRoot, "docs", ".ignored", "notes.md"),
    "# Ignored\n",
  );
  await fs.writeFile(path.join(tempRoot, "skill", "SKILL.md"), "# Skill\n");
  await fs.writeFile(
    path.join(tempRoot, "test", "guard.test.mjs"),
    'import test from "node:test";\n',
  );

  const scannedFiles = await collectScannedFiles(tempRoot);
  const relativeFiles = scannedFiles
    .map((file) => path.relative(tempRoot, file))
    .sort();
