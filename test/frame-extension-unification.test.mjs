...
test("reproduce inconsistency in frame file extension filtering", async (t) => {
  ...
  await t.test("countFrameFiles includes png, jpg, jpeg (case-insensitive)", () => {
    const count = countFrameFiles(framesDir);
    assert.strictEqual(count, 4, "countFrameFiles should count all frame types");
  });

  await t.test("listFrameFiles includes png, jpg, jpeg (case-insensitive)", async () => {
    ...
    assert.strictEqual(files.length, 4, "listFrameFiles should find all 4 frames");
  });

  await t.test("listFrameFilesSync includes png, jpg, jpeg (case-insensitive)", () => {
    const files = listFrameFilesSync(framesDir);
    assert.ok(files.includes("frame-0001.png"));
    assert.ok(files.includes("frame-0002.jpg"));
    assert.ok(files.includes("frame-0003.jpeg"));
    assert.ok(files.includes("frame-0004.PNG"));
    assert.strictEqual(files.length, 4, "listFrameFilesSync should find all 4 frames");
  });

  await t.test("cleanupFrames includes png, jpg, jpeg (case-insensitive)", () => {
    const result = cleanupFrames(tmpDir);
    assert.strictEqual(result.removed, 4, "cleanupFrames should remove all 4 frames");
  });

  await t.test("copySamplesSync preserves original file extensions", () => {
    const tmpDir2 = fs.mkdtempSync(path.join(os.tmpdir(), "timelapse-repro-311-samples-"));
    ...
    assert.ok(result.samples[0].endsWith(".jpg"), "Sample should preserve .jpg extension");
    assert.ok(fs.existsSync(path.join(tmpDir2, "samples", "sample-000001.jpg")), "Sample file should exist with .jpg extension");
